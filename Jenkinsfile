...
timeout(1200){
    node("maven") {
        try {
            ...
            stage("Running rest wiremock Automation") {

                def status = sh(
                        script: "docker run --name=wiremock_tests -e BASE_URL=$base_url --network=host -v \$PWD/allure-results:/app/allure-results localhost:5005/wiremock_tests:latest",
                        returnStatus: true
                )
            ...
            def messageContent = """====WIREMOCK TESTS RESULT ====
                base_url: ${base_url}
